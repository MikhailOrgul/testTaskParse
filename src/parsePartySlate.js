...
const getParseData = async () => {
    ...
    while(agenciesList.length < 50){
        ...
        for(let i = 0; i < cardsArticlesCount; i++){
            console.log('[INFO]', `Страница ${pageCount}; Агенство ${i+1}`)
            ...
            if(agenciesList.length >= 50) break
        }
    ...
    const newAgenciesList = agenciesList.map(item => ({ //Для xlsx чтобы, по быстрому, чтобы не переделывать
        companyName: item.companyName,
        location: item.location,
        phoneNumber: item.contacts.phoneNumber,
        website: item.contacts.website,
        instagram: item.contacts.instagram,
